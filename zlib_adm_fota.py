...
def test(new_bc_slot):
    
    record = fota.get_record()
    #new bytecode, current vm
    
    fota.attempt(new_bc_slot,record[1])
...
def is_fota_valid(metadata):
    ...
    try:
        record = fota.get_record()
        if metadata["bc_slot"]==record[4] and metadata["bc_slot"]!=record[5]:
    ...
    return False,"fota slot not valid"
    
    

# ???
def finalize_fota(status):
    if status == "done":
        fota.accept()
